# Architecture — plugin-analyzer

## System Overview

plugin-analyzer is a Claude Code plugin that scans installed plugins, decomposes them into capabilities, scores each capability, and maintains a registry of per-form winners. It operates as a **read-only analyzer** of the plugin cache — it never modifies installed plugins.
...
## Plugin Overrides — Managed by plugin-analyzer
...
│    Find all      │  Filter: skip plugin-analyzer itself
...
5. Filter out: `plugin-analyzer` (self)
